 ...
 *
 */
function ikon_javascript() {
global $shortname;
$nonce= wp_create_nonce  ('my-nonce');
?>
<?php include( TEMPLATEPATH . '/admin/js/checkbox.php'); ?>
...
add_action('wp_ajax_remove_header_image', 'ikon_remove_header_image_callback');
/**
 * Process our Ajax request to remove the header image
 *
 */
function ikon_remove_header_image_callback() {
$nonce=$_REQUEST['_nonce'];
if (! wp_verify_nonce($nonce, 'my-nonce') ) die('Nice Try');
if (! ikon_can_edit_theme_options() ) die('Nice Try');
