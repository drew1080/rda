<?php
add_action('admin_init', 'ikon_add_init');
add_action('admin_menu' , 'ikon_add_admin');
/*
 * Create theme panel.
 */
function ikon_add_admin() {
  global $themename, $shortname, $moboptions;
  if ($_GET['page'] == basename(__FILE__)) {
  if (! ikon_can_edit_theme_options() ) wp_die('Nice Try');
    ...
    $optionspage =  add_menu_page($themename, $themename, 'administrator', basename(__FILE__), 'ikon_admin', $file_dir."/admin/images/iphone-image.png");
	
	add_action('admin_print_styles-' . $optionspage, 'ikon_admin_print_styles');
	add_action('admin_head-' . $optionspage, 'ikon_javascript');

 // Default
}
function ikon_add_init() {
$file_dir=get_bloginfo('template_directory');
wp_enqueue_style("functions-mob", $file_dir."/admin/admin_panel_styles.css", false, "1.0", "all");
wp_enqueue_script('jquery');
wp_enqueue_script( 'tabs', $file_dir.'/admin/js/tabs.js', array( 'jquery' ) );
wp_enqueue_script( 'colorpicker-jq', $file_dir.'/admin/js/colorpicker.js', array( 'jquery' ) );
wp_enqueue_script( 'jquery-tools', $file_dir.'/admin/js/jquery.tools.min.js', array( 'jquery' ) );

}
function ikon_admin_print_styles(){
	wp_enqueue_style('ikon_style');
}
...
function ikon_admin() {
  ...
  if ($_REQUEST['saved']) {
    echo '<div id="message" class="updated fade">' . $themename . ' ' . __('settings saved.') . '</div>';
  }
  if ($_REQUEST['reset']) {
    echo '<div id="message" class="updated fade">' . $themename . ' ' . __('settings reset.') . '</div>';
  }
  if ($_REQUEST['reset_widgets']) {
    echo '<div id="message" class="updated fade">' . $themename . ' ' . __('widgets reset.') . '</div>';
  }
  if ($_REQUEST['error']) {
...
<!--div class="hot-links">
	<a class="theme-logo" href="#"><img src="<?php echo IWBC_URI; ?>/admin/images/gallant-logo.png" /></a>    
</div>
<a class="twitme" href="http://twitter.com/webincode"><img src="<?php echo IWBC_URI; ?>/admin/images/twitter.png" /></a-->
 ...
 */
function ikon_admin_get_my_options() {
  global $themename, $shortname, $moboptions;
  $i=0;
  $moboptions = apply_filters('ikon_options_list', $moboptions);
  foreach ($moboptions as $value) {
    switch ($value['type']) {
      case "open":
?>
...
<div class="icon-holder"><a href="#"><img src="<?php echo get_bloginfo('template_directory'); ?>/images/social_icons/<?php echo get_option($value['id']); ?>" alt="" /></a></div>
 ...
 <small><?php echo $value['desc']; ?></small><div class="clearfix"></div>
      ...
      case 'upload':
       ...
       <div class="rm_input rm_upload">
          <label for="<?php echo $value['id']; ?>"><?php echo $value['name']; ?></label>
		  <div class='imgupload'>
            <?php print ikon_get_upload_field($value['id'], $value['std'], $value['desc']); ?>
			</div>
