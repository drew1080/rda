...
add_shortcode( 'tab', 'wbc_tab' );
add_shortcode( 'tabs', 'wbc_tabs' );
add_shortcode( 'toggle_alt', 'wbc_toggle' );
add_shortcode( 'toggle', 'wbc_toggle_alt' );
add_shortcode( 'accordion', 'wbc_accordion' );
add_shortcode( 'accn', 'wbc_accn' );
add_shortcode('one_half', 'half');
add_shortcode('one_half_last', 'half_last');
add_shortcode('divider', 'wbcdivider');
add_shortcode('highlight', 'highlightit');
add_shortcode('pullquote', 'pullquoteit');
add_shortcode('blockquote', 'blockquoteit');
add_shortcode("list", "customlistit");
add_shortcode('dropcap', 'wbcdropcap1');
add_shortcode('btn', 'ikn_buttons');
add_shortcode('image_framed', 'imageframed');
add_shortcode('qr_code', 'wcsp_qr_code_shortcode_handler');

add_filter('widget_text', 'do_shortcode');
add_filter('widget_about_text', 'do_shortcode');


// Jquery Tabs
function wbc_tabs( $atts, $content ){
	$GLOBALS['tab_count'] = 0;
	...
	if( is_array( $GLOBALS['tabs'] ) ){
		foreach( $GLOBALS['tabs'] as $tab ){
			$tabs[] = '<li><a class="" href="#"><span>'.$tab['title'].'</span></a></li>';
...
function wbc_toggle($atts, $content = null) {
	extract (shortcode_atts(array(
		'title' => ''), 
		$atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
		return '<h3 class="trigger"><a href="#">'.$title.'</a></h3><div class="toggle_container"><div class="block">'.do_shortcode($content).'</div></div>';
}
function wbc_toggle_alt($atts, $content = null) {
	extract (shortcode_atts(array(
		'title' => ''), 
		$atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
		return '<h3 class="trigger alt"><a href="#">'.$title.'</a></h3><div class="toggle_container"><div class="block">'.do_shortcode($content).'</div></div>';
}

// Horizontal accordion 

function wbc_accordion($atts, $content = null) {
	$content = preg_replace('#^<\/p>|<p>$#', '', $content);
	return 	'<div class="tabs_accordion">'. do_shortcode($content) .'</div>';
}
function wbc_accn($atts, $content = null) {
	extract (shortcode_atts(array(
		'img' => '',
		'title' => ''), 
		$atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
		return '<h3 class="acc_trigger"><a href="#">'.$title.'</a></h3><div class="acc_container"><div class="block">'. do_shortcode($content) .'</div></div>';
}
...
//Divider 
function wbcdivider($atts, $content = null) {
	$content = preg_replace('#^<\/p>|<p>$#', '', $content);
...
function highlightit($atts, $content = null) {
	extract(shortcode_atts(array(
		'color'		=> 'black'), $atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
...
function pullquoteit($atts, $content = null) {
	extract(shortcode_atts(array(
		'position'		=> 'left'), $atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
		return '<span class="pullquote-'.$position.'">'.do_shortcode($content).'</span>';
}

//Blockquote

function blockquoteit($atts, $content = null) {
	extract(shortcode_atts(array(
		'author'		=> '&nbsp;'), $atts));
		$content = preg_replace('#^<\/p>|<p>$#', '', $content);
...
function customlistit($atts, $content = null) {
	extract(shortcode_atts(array(
		'color'		=> ' ', 
		'type'		=> 'bullet'),
		$atts));
	$content = str_replace('<ul>', '<ul class="list '.$color.' '.$type.'">', do_shortcode($content));
	$content = preg_replace('#^<\/p>|<p>$#', '', $content);
	return $content;
}

//Dropcap 1
function wbcdropcap1($atts, $content = null) {
	extract (shortcode_atts(array(
		'color' => ''),
		$atts));
	$content = preg_replace('#^<\/p>|<p>$#', '', $content);
...
function ikn_buttons($atts, $content = null) {
    extract(shortcode_atts(array(
        'text'     => 'Link label',
        'id'     => '1',
        'url'    => '',
    ...
    if($p!='false') : return wpautop('<a href="'.$link.'" class="button '.$color.'" '.$target.'><span>'.do_shortcode($content).'</span></a>');
    else : return '<a href="'.$link.'" class="button '.$color.'" '.$target.'><span>'.do_shortcode($content).'</span></a>';
    endif;
}
...
function imageframed($atts, $content = null) {
	extract(shortcode_atts(array(
	"src"=>'',
	"alt"=>'',
	"align"=>''), $atts));
	$content = preg_replace('#^<\/p>|<p>$#', '', $content);
	if ($align) :
			return '<img class="align'.$align.' framed" src="'.$src.'" alt="'.$alt.'" />';
		else :
			return '<img class="framed" src="'.$src.'" alt="'.$alt.'" />';
		endif;
}
