...
define("BITCASA_BASE_URL", "https://developer.api.bitcasa.com/v1");
...
class BitcasaClient_WP extends BitcasaClient {
	...
	protected function http_delete($url, $args = NULL, $body = NULL)
	{
		...
		$opts = array('method' => 'DELETE', 'timeout' => 45);
		if ($body != NULL) {
			$opts['body'] = $body;
		}
		if (UpdraftPlus_Options::get_updraft_option('updraft_ssl_disableverify')) $opts['sslverify'] = true;
		...
		$r = wp_remote_request($full_url, $opts);
		if (is_wp_error($r)) {
			global $updraftplus;
			$updraftplus->log_wp_error($r, false, true);
			throw new BitcasaException("Invalid response code", 500);
		}
	...
	protected function http_post($url, $args = NULL, $body = NULL)
	{
		$full_url = $this->base_url . $url;
		$first = true;
		...
		$opts = array('timeout' => 45, 'body' => $body);
		if (UpdraftPlus_Options::get_updraft_option('updraft_ssl_disableverify')) $opts['sslverify'] = true;
		if (!UpdraftPlus_Options::get_updraft_option('updraft_ssl_useservercerts')) $opts['sslcertificates'] = UPDRAFTPLUS_DIR.'/includes/cacert.pem';

		$r = wp_remote_post($full_url, $opts);
		if (is_wp_error($r)) {
			global $updraftplus;
			$updraftplus->log_wp_error($r, false, true);
			throw new BitcasaException("Invalid response code", 500);
	...
	# Public: we want to call it directly
	public function http_get_file($url, $args = array(), $headers = array()) {
		$full_url = $this->base_url . $url;
		$first = true;
		...
		$opts = array('timeout' => 300, 'headers' => $headers);
		if (UpdraftPlus_Options::get_updraft_option('updraft_ssl_disableverify')) $opts['sslverify'] = true;
		if (!UpdraftPlus_Options::get_updraft_option('updraft_ssl_useservercerts')) $opts['sslcertificates'] = UPDRAFTPLUS_DIR.'/includes/cacert.pem';

		$r = wp_remote_get($full_url, $opts);
		if (is_wp_error($r)) {
			global $updraftplus;
			$updraftplus->log_wp_error($r, false, true);
			throw new BitcasaException("Invalid response code", 500);
	...
	protected function http_get($url, $args = array())
	{
		$full_url = $this->base_url . $url;
		$first = true;
		...
		$r = wp_remote_get($full_url, array('timeout' => 30));
		if (is_wp_error($r)) {
			global $updraftplus;
			$updraftplus->log_wp_error($r, false, true);
			throw new BitcasaException("Invalid response code", 500);
	...
	protected function http_put_file($url, $args = array(), $filename, $filepath)
	{
...
class BitcasaClient
{
	...
	protected $base_url;
	protected $infinite_drive;
	protected $mirrored_folders;
	# UD addition
	public $result;
...
// 
	...
	protected function http_get_file($url, $args = array())
	{
		$full_url = $this->base_url . $url;
		$first = true;
	...
	protected function encodeArray($data, $for_get = false)
	{
		// todo: encode
		if ($data == NULL) {
			$data = array();
