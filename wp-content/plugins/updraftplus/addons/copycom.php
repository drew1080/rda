...
/* TODO
- Download isn't being updated (the PageVisibility)
*/
...
class UpdraftPlus_Addons_RemoteStorage_copycom extends UpdraftPlus_RemoteStorage_Addons_Base {
	...
	public function do_upload($file, $from) {
		...
		if ($this->next_partno > 0) $updraftplus->log($this->description.": Some chunks have already been uploaded; next chunk number is: ".$this->next_partno);
	...
	public function chunked_upload($file, $fp, $i, $upload_size, $upload_start, $upload_end) {
		...
		if ($i < $this->next_partno+1 && isset($this->parts[$this->next_partno])) {
			# Already uploaded
		...
		try {
			...
			$this->parts[$this->next_partno] = $part;
			$this->next_partno++;
	...
	*/
	...
	public function do_config_print($opts) {
		...
		$updraftplus_admin->storagemethod_row(
			'copycom',
			'',
			'<img src="'.UPDRAFTPLUS_URL.'/images/copycom.png">'.$apikey_text
		);
		...
		if (defined('UPDRAFTPLUS_COPYCOM_CUSTOMKEYS') && true == UPDRAFTPLUS_COPYCOM_CUSTOMKEYS) {
			...
			$updraftplus_admin->storagemethod_row(
				...
				'<input type="'.apply_filters('updraftplus_admin_secret_field_type', 'text').'" style="width:442px" name="updraft_copycom[secret]" value="'.esc_attr($secret).'">'
			);
		} else {
