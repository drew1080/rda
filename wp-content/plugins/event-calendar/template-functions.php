  ...
  $month_format  =EC3_DEFAULT_MONTH_FORMAT)
{
  ...
  if($calendar_entries)
  {
    ...
    foreach($calendar_entries as $entry)
    {
	  ...
	  $rdadata = get_the_category($entry->id);
	  
	  $rdacheck = array();
	  foreach ($rdadata as $rdasel) {
	  if ($rdasel->cat_ID == 16) { continue; }
	  $rdacheck[] = $rdasel->cat_ID; }
	  
	  if (in_array($rdaselect,$rdacheck)) { continue; }
	  
	  
      echo " <li class='";
	  foreach ($rdadata as $rdaecc) { echo 'ecc' . $rdaecc->cat_ID . ' '; }
	  echo "'>".ec3_format_str($template_event,$data)."</li>\n";
    }
    if($current_date)
        echo "</ul></li>\n";
    if($current_month)
