	...
	class GA_Admin extends Yoast_GA_Plugin_Admin {
				 ...
				 <script type="text/javascript">				
					function makeSublist(parent,child,childVal)
					{
				 	...
				 	jQuery(document).ready(function(){
						...
						jQuery('#gajsricedesignalliance.orging').change(function(){
							if ((jQuery('#gajsricedesignalliance.orging').attr('checked')) == true)  {
								jQuery('#ricedesignalliance.orgingbox').css("display","block");
							} else {
								jQuery('#ricedesignalliance.orgingbox').css("display","none");
							}
						}).change();
		...
		function save_settings() {
			...
			if ( isset($_REQUEST['reset']) && $_REQUEST['reset'] == "true" && isset($_REQUEST['plugin']) && $_REQUEST['plugin'] == 'google-analytics-for-wordpress') {
				...
				foreach (array('extrase', 'trackoutbound', 'admintracking', 'trackadsense', 'allowanchor', 'allowlinker', 'allowhash', 'rsslinktagging', 'advancedsettings', 'trackregistration', 'theme_updated', 'cv_loggedin', 'cv_authorname', 'cv_category', 'cv_all_categories', 'cv_tags', 'cv_year', 'cv_post_type', 'outboundpageview', 'downloadspageview', 'gajsricedesignalliance.orging', 'manual_uastring', 'taggfsubmit', 'wpec_tracking', 'shopp_tracking', 'anonymizeip', 'trackcommentform', 'debug','firebuglite') as $option_name) {
						...
						<div class="meta-box-sortables">
							<form action="<?php echo $this->plugin_options_url(); ?>" method="post" id="analytics-conf">
								...
								<?php
									...
									$rows[] = array(
										'id' => 'gajsricedesignalliance.orging',
										'label' => 'Host ga.js locally',
										'content' => $this->checkbox('gajsricedesignalliance.orging').'<div id="ricedesignalliance.orgingbox">
...
/*									if (class_exists('RGForms') && GFCommon::$version >= '1.3.11') {
		...
		function set_defaults() {
			$options = array(
				...
				'gajsricedesignalliance.orging'		=> false,
				'gajsurl'				=> '',
				'ignore_userlevel'		=> '11',
				'internallink'			=> false,
				'internallinklabel'		=> '',
...
?>
	...
	(function() {
		...
		ga.src = <?php 
if ( $options['gajsricedesignalliance.orging'] && !empty($options['gajsurl']) ) {
