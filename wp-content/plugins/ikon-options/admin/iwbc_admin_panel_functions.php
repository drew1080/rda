...
function iwbc_get_upload_field($id, $std = '', $desc = '') {
  $data = get_option($id);
  ...
  $field = '<input id="' . $id . '" type="file" name="attachment_' . $id . '" />' .
           ...
           ($data ?
				iwbc_get_upload_image_preview($data) .
 ...
 */
function iwbc_get_upload_image_preview($data = '') {
  if (!empty($data)) {
 ...
 *
 */
function iwbc_can_edit_theme_options(){
	global $wp_version;
